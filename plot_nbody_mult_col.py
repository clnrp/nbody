...
def init():
    ax.set_xlim((-Ds, Ds))
    ax.set_ylim((-Ds*.3, Ds*.3))
    ax.set_zlim((-Ds*.3, Ds*.3))
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.view_init(30, 112)
...
if(larg==3 and cmd=='r'):
   ...
   Pp=array([-Ds,0,0])
   Vp=array([.9,0,0])
   s1=mult_stars(Pp,Vp,Mp*.9,Dp,N1,sn)

   # sistema 2
   Pp=array([Ds,0,0])
   Vp=array([-.8,0,0])
   [m,x0,v0]=mult_stars(Pp,Vp,Mp*.95,Dp,N2,s1)
   mlist=[m,x0,v0]
   print('gravar!')
   if(larg==3 and cmd=='w'):
...
#anim.save('ani.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
plt.show()
